...
let retrieved_user: User|undefined
  ...
  // Take pin from the user for login
        ...
        retrieved_user = search_user(entered_pin.pin);
                    if (retrieved_user) 
                    {
                        ...
                        console.info(
                        chalk.yellowBright(
                            `Welcome ${authenticatedUser}. Your Acc #: ${retrieved_user.Account_No}, Balance: ${retrieved_user.Balance} `
                        )
                        );
                        console.info(
                        `------------------------------------------------------------------------ `
...
//
function search_user(_Pin: number): User | undefined {
  // Assuming users is an array of type User
  let result: User | undefined = users.find((user: User) => user.Pin === _Pin);
  return result;
}

async function loggedin_user_operations(retrieved_user: User, isauthenticated: boolean) {
  //display main menu
...
async function Transfer_Balance(_sender_account: User)
{
    ...
    let receipent: User|undefined = users.find((user: User) => user.Account_No === answer.Acc);
    if (receipent) 
    {
    console.log("\n");
